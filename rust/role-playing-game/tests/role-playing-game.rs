...
fn test_reviving_dead_player() {
  let dead_player = Player { health: 0, mana: Some(0), level: 34 };
...
fn test_reviving_dead_level9_player() {
  let dead_player = Player { health: 0, mana: None, level: 9 };
  let revived_player = dead_player.revive().expect("reviving a dead player must return Some(player)");
  assert_eq!(revived_player.health, 100);
...
fn test_reviving_dead_level10_player() {
  let dead_player = Player { health: 0, mana: Some(0), level: 10 };
  let revived_player = dead_player.revive().expect("reviving a dead player must return Some(player)");
  assert_eq!(revived_player.health, 100);
  assert_eq!(revived_player.mana, Some(100));
  assert_eq!(revived_player.level, dead_player.level);
...
fn test_reviving_alive_player() {
  let alive_player = Player { health: 1, mana: None, level: 8 };
...
fn test_cast_spell_with_enough_mana() {
  ...
  let mut accomplished_wizard = Player { health: HEALTH, mana: Some(MANA), level: LEVEL };
...
fn test_cast_spell_with_insufficient_mana() {
  let mut no_mana_wizard = Player { health: 56, mana: Some(2), level: 22 };

  // we want to clone so we can compare before-and-after effects of casting the
  // spell, but we don't want to introduce that concept to the student yet, so
...
fn test_cast_spell_with_no_mana_pool() {
  ...
  let mut underleveled_player = Player { health: 87, mana: None, level: 6 };
...
fn test_cast_large_spell_with_no_mana_pool() {
  ...
  let mut underleveled_player = Player { health: 20, mana: None, level: 6 };
